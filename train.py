...
from trainer.callbacks.base import (
...
parser.add_argument(
    "--artifact", type=str, default=None, help="Artifact's name to load the model from."
)
args = parser.parse_args()

print("Training Configuration:")
print(
    f"Learning Rate: {args.lr}, Model: {args.model}, Batch Size: {args.bs}, Epochs: {args.epochs}, FP16: {args.fp16}"
)
...
class_weights, class_labels, class_names = get_loss_class_weights(train_file)
...
model = get_model(name=args.model, new_head=new_head)
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, factor=0.1, mode="max", patience=4, min_lr=1e-7
)
callbacks = [
...
run = wandb.init(
    ...
    config={
        ...
        "loss": f"FocalLoss(weighted), gamma={args.gamma}",
...
if args.artifact:
    artifact_to_load = run.use_artifact(
        f"{WANDB_ENTITY}/{WANDB_PROJECT}/{args.artifact}"
    )
